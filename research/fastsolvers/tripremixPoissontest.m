function [sigma,u,info] = tripremixPoissontest(M,B,C,f,g,elem,option)
...
[x,flag,stopErr,itStep,err] = minres(bigA,bigF,tol,maxIt,@DBC,[],x0);
...
%         du = amg(Abar,rg - B*ds,option);
        du = 0.25*mg(Abar,rg - B*ds,elem,option,Ai,Bi,BBi,Res,Pro,isFreeDof);
        s = [ds - DMinv*(Bt*du); du];
    end
end