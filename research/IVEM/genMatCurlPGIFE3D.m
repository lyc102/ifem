function matrix = genMatCurlPGIFE3D(pde,mesh,fem,femI,option)
...
%% 1. Stiffness Matrix
S = globMatrixNedPGIFE3D(pde.A,1,mesh,femI,fem,fem);
M = globMatrixNedPGIFE3D(pde.B,0,mesh,femI,fem,fem);

%% 2. Generate the Right Hand Side Vector
rhsF1 = globNedRHSPGIFE3D(pde.f1, mesh, fem, femI, 0, 1);
rhsF2 = globNedRHSPGIFE3D(pde.f2, mesh, fem, femI, 0, 2);
rhsF3 = globNedRHSPGIFE3D(pde.f3, mesh, fem, femI, 0, 3);
rhsF = rhsF1 + rhsF2 + rhsF3;

%% 3. Dirichlet Boundary Conditions
if isfield(option,'HM')
    if option.HM == 1
        Atotal = S - pde.kappa*M;
        BP = S + pde.kappa*M;
    end
else
     Atotal = S + M;
