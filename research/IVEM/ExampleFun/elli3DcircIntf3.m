function pde = elli3DcircIntf3(am,ap,bm,bp,r1,r2,n1,n2)
...
%% exact solution
    ...
    function u = um1(x,y,z)
        u = x/bm + n1*uker1(x,y,z).*(y-z)/am;
    end
    function u = um2(x,y,z)
        u = y/bm + n1*uker1(x,y,z).*(z-x)/am;
    end
    function u = um3(x,y,z)
        u = z/bm + n1*uker1(x,y,z).*(x-y)/am;
    end
    function u = up1(x,y,z)
        u = x/bp + n2*uker1(x,y,z).*uker2(x,y,z).*(y-z)/ap;
    end
    function u = up2(x,y,z)
        u = y/bp + n2*uker1(x,y,z).*uker2(x,y,z).*(z-x)/ap;
    end
    function u = up3(x,y,z)
        u = z/bp + n2*uker1(x,y,z).*uker2(x,y,z).*(x-y)/ap;
    end
