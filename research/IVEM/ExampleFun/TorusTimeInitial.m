function pde = TorusTimeInitial(mum,mup,sigm,sigp,epsm,epsp,omega,x0,y0,z0,r1,r2,r)
...
rr = 3;
    function u = E1(x,y,z)
        u = (exp(-L2Norm(x,y,z)/r^2)-exp(-rr^2)).*x;
        id = pulregion(x,y,z) > 0;
        u(id) = zeros(size(x(id)));
    end
    function u = E2(x,y,z)
        u = (exp(-L2Norm(x,y,z)/r^2)-exp(-rr^2)).*y;
        id = pulregion(x,y,z) > 0;
        u(id) = zeros(size(x(id)));
    end
    function u = E3(x,y,z)
        u = (exp(-L2Norm(x,y,z)/r^2)-exp(-rr^2)).*z;
        id = pulregion(x,y,z) > 0;
        u(id) = zeros(size(x(id)));
    end
    function u = Et1(x,y,z)
        u = -Sig(x,y,z)./Epslon(x,y,z).*E1(x,y,z);
    end
    function u = Et2(x,y,z)
        u = -Sig(x,y,z)./Epslon(x,y,z).*E2(x,y,z);
    end
    function u = Et3(x,y,z)
        u = -Sig(x,y,z)./Epslon(x,y,z).*E3(x,y,z);
    end
    function u = pulregion(x,y,z)
       u = (x.^2+y.^2+z.^2).^2/(rr*r)^2 - 1; 
    end
