function pde = LinearFun(am,ap,bm,bp,r1,r2,r3,x0,y0,z0)
...
%% interface function
    function u = intf(x,y,z)
        u = (x-x0);%(x-x0)*r1 + (y-y0)*r2 + (z-z0)*r3;
    end
...
%% Derivative of the exact solution
    ...
    function u = Dxum(x,y,z)
        u = (r2-r3)*ones(size(x))/am;
    end
    function u = Dyum(x,y,z)
        u = (r3-r1)*ones(size(x))/am;
    end
    function u = Dzum(x,y,z)
        u = (r1-r2)*ones(size(x))/am;
    end
    function u = Dxup(x,y,z)
        u = (r2-r3)*ones(size(x))/ap;
    end
    function u = Dyup(x,y,z)
        u = (r3-r1)*ones(size(x))/ap;
    end
    function u = Dzup(x,y,z)
        u = (r1-r2)*ones(size(x))/ap;
    end
...
%% right hand side function
    ...
    function u = fm1(x,y,z)
        u = ((x-x0)*r1 + (y-y0)*r2 + (z-z0)*r3)*bm/am;
    end
    ...
    function u = fp1(x,y,z)
        u = ((x-x0)*r1 + (y-y0)*r2 + (z-z0)*r3)*bp/ap;
    end
