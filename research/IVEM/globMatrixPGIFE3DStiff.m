function S = globMatrixPGIFE3DStiff(fun,mesh,femI,fem1,fem2)
...
Jbasx = cell(dof2,1); Jbasy = cell(dof2,1); Jbasz = cell(dof2,1);
for i = 1:dof1
    Ibasx{i} = feEvalBas1(fem1.bas(tID,:,i), gxI, gyI, gzI, [1,0,0]);
    Ibasy{i} = feEvalBas1(fem1.bas(tID,:,i), gxI, gyI, gzI, [0,1,0]);
    Ibasz{i} = feEvalBas1(fem1.bas(tID,:,i), gxI, gyI, gzI, [0,0,1]);
end
for j = 1:dof2
