function S = globMatrixNedPGIFE3D(fun,MatInd,mesh,femI,fem1,fem2)
...
Jbasx = cell(dof2,1); Jbasy = cell(dof2,1); Jbasz = cell(dof2,1);
for i = 1:dof1
    Ibasx{i} = feEvalBas1(fem1.bas, tIntfID, gxI, gyI, gzI, i, MatInd, 1).*t_e_orit(tIntfID,i);
    Ibasy{i} = feEvalBas1(fem1.bas, tIntfID, gxI, gyI, gzI, i, MatInd, 2).*t_e_orit(tIntfID,i);
    Ibasz{i} = feEvalBas1(fem1.bas, tIntfID, gxI, gyI, gzI, i, MatInd, 3).*t_e_orit(tIntfID,i);
end
for j = 1:dof2
