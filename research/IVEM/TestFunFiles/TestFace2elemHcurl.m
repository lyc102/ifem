...
for kk = 1:length(une) % group polys according to their # of nodes
    ...
    GPB2 = bm/bp*GPB1; GPB2(:,2,:) = GPB2(:,2,:); GPB2(:,3,:) = GPB2(:,3,:);
...
XN = zeros(nloc*ntN, 1);
%t_e_orit = fem.t_e_orit(mesh.tLoc>0,:);
...
femIH1 =  genP1VIFEM3D2(meshI,mesh,pde,opt);
S = globMatrixVIFE3DStiff2(pde.A,mesh,femH1,femH1);
onefun = @(x,y,z) ones(size(x));
...
%% cutting edges: p1-pint-p2
eIntPID = size(mesh.p,1)+1:size(node,1);
% first piece of cutting edge: p1-pint
CutEdge1NodeID = [mesh.e(mesh.eLoc<0,1),eIntPID'];
[CutEdge1,CutEdge1IDa,CutEdge1IDb] = intersect(gdof,CutEdge1NodeID,'row');
[~,CutEdge1IDbSortID] = sort(CutEdge1IDb);
CutEdge1ID = CutEdge1IDa(CutEdge1IDbSortID,:);
% the index of the first piece, gdof(CutEdge1ID,:) gives the two node index
% for each edge and the second index is always corresponding to the
% intersection point, this is also true for CutEdge2ID
% second piece of cutting edge: p2-pint
CutEdge2NodeID = [mesh.e(mesh.eLoc<0,2),eIntPID'];
[CutEdge2,CutEdge2IDa,CutEdge2IDb] = intersect(gdof,CutEdge2NodeID,'row');
[~,CutEdge2IDbSortID] = sort(CutEdge2IDb);
CutEdge2ID = CutEdge2IDa(CutEdge2IDbSortID,:);  % the index of the second piece
% check whether they have the same direction
vt1 = node(gdof(CutEdge1ID,2),:) - node(gdof(CutEdge1ID,1),:);
vt2 = node(gdof(CutEdge2ID,2),:) - node(gdof(CutEdge2ID,1),:);
CrossPrd = cross(vt1,vt2); % should be zero
%% new edge added inside each face
EdgeinFaceID = false(NEdof,1);
EdgeinFaceID(BasicEdgeNum+1:NEdof) = true;
EdgeinFaceID(CutEdge1ID) = false;
EdgeinFaceID(CutEdge2ID) = false;
otherEdgeID = ~EdgeinFaceID;
...
option.smoother = 'BD';
option.cutEdge = [CutEdge1ID, CutEdge2ID];
...
% option.M = M;
% [x,info] = amgMaxwellTest(A,f,node,edge,option);
[x,info] = amgMaxwellinterface(A,f,node,edge,option);
