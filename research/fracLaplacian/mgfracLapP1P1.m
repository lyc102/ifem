function [x,info] = mgfracLapP1P1(A,b,elem,option,varargin)
%% MGFRACLAPP1P1 multigrid solvers for fractional Laplacian
%
%
% See also mg
%
...
Ny = Ndof/N;                    % number of nodes in the extend direction
...
for j = 1:level-1
    Pro{j} = kron(speye(Ny-1,Ny-1),Pro{j}); % for each level in y direction
    Res{j+1} = Pro{j}';
end
clear HB
...
for j = level:-1:2
    ...
    switch option.smoother
        ...
        case 'LINE'
            ...
            d1 = full(diag(Ai{j}));
            d2 = zeros(NxNy,1);
            d2(1:NxNy-Nx) = full(diag(Ai{j},Nx));
            d3 = zeros(NxNy,1);
            d3(Nx+1:NxNy) = d2(1:NxNy-Nx);
            Bi{j} = spdiags([d2 d1 d3],[-Nx 0 Nx],NxNy,NxNy);
            L{j} = chol(Bi{j},'lower');
            R{j} = L{j}';
