function [u,eqn,info] = fracLapP2bP2(node,elem,pde,option)
%% FRACLAPP2BP2 solves fractional Laplacian equation using P2b-P2 element
%
...
NTtotal = NT*NTy;
Nxdof = (N + NE + NT); % dof in x  is number of vertices plus edges and triangles
Nydof = (Ny + NTy);
Ndof = Nxdof*Nydof;
elem2dof(:,7) = N + NE + (1:NT); % add elementwise bubble
...
elseif isfield(pde,'f') && ischar(pde.f)
    if strcmp(pde.f,'intx') % f is a distribution 
        ...
        [lambda,weightf] = quadpts(option.gNquadorder);
        nQuad = size(lambda,1);
        [Dlambda,area] = gradbasis(node,elem);
        for p = 1:nQuad
            % Dphi at quadrature points
            ...
            pxy = lambda(p,1)*node(elem(:,1),:) ...
                ...
                + lambda(p,3)*node(elem(:,3),:);            
            for i = 1:7
                ft(:,i) = ft(:,i) + weightf(p)*Dxphip(:,i).*pde.intx(pxy);
            end
        end
    end
end
...
switch solver
    ...
    case 'mg'
        ...
        [u,info] = mgfracLapP2bP2(A,b,elem,option);
    case 'amg'
        option.solver = 'CG';
