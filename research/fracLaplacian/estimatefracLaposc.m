function [eta,estar,oscf] = estimatefracLaposc(node,elem,y,u,pde,option)
...
for i = 1:N
    ...
    if isBdNode(i) % for bd nodes, it is not included in the local dof
        starDof = dofMap([1:Ny-1,Ny+1:Nydof],starEdges+N);
    else
        starDof = dofMap([1:Ny-1,Ny+1:Nydof],[i; starEdges+N]);
    end
