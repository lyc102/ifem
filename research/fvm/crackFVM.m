...
for k = 1:maxIt
    % Step 1: SOLVE
    u = PoissonFVM(node,elem,pde,bdEdge);
    ...
    % eta = effrecflux(node,elem,pde,bdEdge,u);   % lcoal L2 project
...
figure(2);
r1 = showrate(N,errH1,2,'g-*');
hold on;
r2 = showrate(N,errL2,2,'k-+');
legend('||Du-Du_h||',['cN^{' num2str(r1) '}'], ...
       '||u-u_h||',['cN^{' num2str(r2) '}'], ...
       'LOCATION','Best');
%%
