function pde = sincosdata3Neumann
%% SINCOSDATA3NEUMANN trigonometric data for Poisson equation in (0,1)^3
%
...
%     Du/dn is nonzero on 6 boundary faces.
%
...
pde = struct('f',@f,'exactu',@exactu,'g_D',@g_D,'Du',@Du,'g_N',@g_N, 'phi', @phi);
    ...
    function s = f(p)
    x = p(:,1); y = p(:,2); z = p(:,3);
    s = 3*pi^2*sin(pi*x).*sin(pi*y).*sin(pi*z);
    end
    % exact solution
    function s = exactu(p)
    x = p(:,1); y = p(:,2); z = p(:,3);
    s = sin(pi*x).*sin(pi*y).*sin(pi*z) - 8/pi^3; % for neumann boundary condition, int_u =0
    end
    ...
    f = zeros(size(p,1),1);
    x = p(:,1); y = p(:,2); z = p(:,3);
    uprime = [pi*cos(pi*x).*sin(pi*y).*sin(pi*z) ...
              pi*sin(pi*x).*cos(pi*y).*sin(pi*z) ...
              pi*sin(pi*x).*sin(pi*y).*cos(pi*z)];
    ...
    function s = Du(p)
    x = p(:,1); y = p(:,2); z = p(:,3);
    s(:,1) = pi*cos(pi*x).*sin(pi*y).*sin(pi*z);
    s(:,2) = pi*sin(pi*x).*cos(pi*y).*sin(pi*z);
    s(:,3) = pi*sin(pi*x).*sin(pi*y).*cos(pi*z);
    end
