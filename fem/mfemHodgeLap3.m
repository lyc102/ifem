function [err,time,solver,eqn] = mfemHodgeLap3(mesh,pde,option,varargin)
...
for k = 1:maxIt
    ...
    switch elemType
        case 'ND0'  % lowest order Nedelec element
            [sigma,u,eqn,info] = HodgeLaplacian3E(node,elem,bdFlag,pde,option);
        case 'RT0'  % RT0 mixed FEM 
            [sigma,u,eqn,info] = HodgeLaplacian3F(node,elem,bdFlag,pde,option);
    end
    % compute error
    t = cputime;
