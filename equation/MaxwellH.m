function [u,edge,eqn,info] = MaxwellH(node,elem,HB,pde,bdFlag,option)
%% MAXWELLH Maxwell equation for H: lowest order edge element.
%
% u = MaxwellH(node,elem,HB,pde,bdFlag) produces the lowest order edge
...
for i = 1:6
    for j = i:6
        ...
        idx = (signij == -1); % insistent local direction
        % curl-curl matrix
...
clear pxyz Jp bt rhs phi_k curlPhi
