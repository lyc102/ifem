...
The convergence is 
<img src="https://latex.codecogs.com/svg.latex?\Large&space;\left\|\nabla\left(u-u_{h}\right)\right\|_{L^{2}(\Omega)}+\epsilon^{-1 / 2}\left\|u-u_{h}\right\|_{L^{2}(\Gamma)} \leq C h^{s-1}\left(1+h_{\Gamma}^{1 / 2} \epsilon^{-1 / 2}\right)\|u\|_{H^{s}(\Omega)}" title="Convergence" />

The condition number is 
<img src="https://latex.codecogs.com/svg.latex?\Large&space;\kappa=\mathcal{O}\left(h^{-2}+(\epsilon h)^{-1}\right)" title="Condition number" />
