function [isC,G] = coarsenAMGc(A,theta)
...
% definition.
%
% [isC,G] = coarsenAMGc(A,theta) accepts the parameter theta to define the
% strong connectness. The default setting is theta = 0.025. It also outputs
% the strong connectness matrix G which could be used in the constrction
...
G = sparse(im(idx),jm(idx),sm(idx),N,N); % matrix for strong connectness
...
deg = sum(spones(G)); % number of strongly connected neighbors
deg = full(deg');
